...
| ④ Output identifier / PII | `POST /v1/screen/output` | Protected identifiers + credential/PII regex (optional NER) | Deterministic | identifiers **100%**, PII regex **53.5%** (NER tier ~80%) |
...
| Output PII (regex tier) | **53.5%** | structured items, zero FP; NER tier ~**80%** |
...
| Performance (hybrid tier) | p50 **0.1 ms** · p99 **34 ms** · **230 QPS / worker** | cascade in effect |
...
| Functional · PII | regex **53.5%** / optional NER tier ~80% | all high-risk credentials/keys/card numbers caught; names/addresses via the optional NER tier |
...
- Adversarial robustness is **not** at 100% — nested encodings and real GCG-optimized suffixes can still bypass; Chinese detection relies on regex; output PII at the regex tier is 53.5% (the NER tier raises it to ~80% at the cost of +1–2 GB image and +50–100 ms latency).
