...
def telemetry() -> dict:
    lat = sorted(TELE["latencies"])
    total = TELE["total"]
    reasons = TELE["reasons"].most_common(8)
    return {
        "total": total,
        "allowed": TELE["allowed"],
        "blocked": TELE["blocked"],
        "block_rate": round(TELE["blocked"] / total, 4) if total else 0.0,
        "would_block": TELE["would_block"],
        "would_block_rate": round(TELE["would_block"] / total, 4) if total else 0.0,
        "wb_timeline": list(TELE["wb_timeline"]),
        "mode": TELE["mode"],
        "by_stage": TELE["by_stage"],
        "latency_ms": {"p50": _pct(lat, 50), "p95": _pct(lat, 95),
                       "mean": round(sum(lat) / len(lat), 3) if lat else 0.0},
        "reasons": [{"name": k, "count": v} for k, v in reasons],
        "timeline": list(TELE["timeline"]),
        "events": list(TELE["events"]),
        "uptime_s": round(time.time() - TELE["started_at"], 1),
    }
