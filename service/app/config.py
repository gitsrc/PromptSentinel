...
def parse_trusted_proxies(raw: Optional[str]):
    """把逗号分隔的 IP/CIDR 列表解析成 ip_network 列表(供 X-Forwarded-For 信任判定)。

    单个 IP(无 /前缀)按 /32(IPv4)或 /128(IPv6)处理。非法条目跳过(不阻断启动,
    仅意味着该条不被信任——安全侧失败)。默认空 ⇒ 返回空列表 ⇒ 一律不信任 XFF。
    """
    nets = []
...
def is_trusted_proxy(client_host: Optional[str], trusted_nets) -> bool:
    """直连对端 client_host 是否落在可信代理网段内。

    trusted_nets 为空 ⇒ 永远 False(安全默认:不信任任何 XFF)。
    client_host 非法/缺失 ⇒ False。
    """
    if not trusted_nets or not client_host:
        return False
    try:
