...
- **输出 PII(④)**:高精度正则档 **~53.5%**(覆盖邮箱/卡号/密钥/GPS/电话/IBAN/日期等结构化项,零误报);姓名/地址等自然语言 PII 需 NER —— 可选 **NER 档**(构建 `WITH_LLM_GUARD=true` + 配置 `use_llm_guard:true`,Presidio)可达 ~80%,代价镜像 +1~2GB。指标为"文本含 PII 的检出率"。
